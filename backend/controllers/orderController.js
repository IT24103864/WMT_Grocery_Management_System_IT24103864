...
const editOrder = async (req, res) => {
  try {
    ...
    if (!isAdmin && (!order.editDeadline || Date.now() > order.editDeadline.getTime())) {
      return res.status(403).json({ message: "Edit window has closed" });
    }

    const { items, deliveryAddress, deliveryDate, deliveryTimeSlot } = req.body;

    if (items && items.length) {
      // FIX #4: validate stock before allowing item quantity changes
      const stockError = await assertOrderStockAvailable(items);
      if (stockError) return res.status(400).json({ message: stockError });
...
const cancelOrder = async (req, res) => {
  try {
    ...
    // FIX #5: admins can cancel any time; customers are bound by the 1-hour window
    if (!isAdmin && Date.now() > order.editDeadline.getTime()) {
      return res.status(403).json({ message: "Edit window has closed" });
    }
